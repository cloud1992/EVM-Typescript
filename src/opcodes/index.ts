...
const Opcodes: {
    ...
    0x51: new Instruction(
        ...
        async (ctx: ExecutionContext) => {
            ...
            return Number(dynamicGas + staticGas);
        },
        (ctx: ExecutionContext) => {
    ...
    0x55: new Instruction(
        ...
        async (ctx: ExecutionContext) => {
            ...
            const storedValue = await ctx.storage.get(setLengthLeft(bigIntToBuffer(key), 32));

            const currentValue = storedValue ? bufferToBigInt(storedValue) : BigInt(0);

            const originalStoredValue = await ctx.originalStorage.get(setLengthLeft(bigIntToBuffer(key), 32));

            const originalValue = originalStoredValue ? bufferToBigInt(originalStoredValue) : BigInt(0);

            const getDynamicGas = () => {
                if (value === currentValue) return 100;
